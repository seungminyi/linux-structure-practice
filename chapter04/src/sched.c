...
static inline long diff_nesc(struct timespec before, struct timespec after)
{
...
static unsigned long loops_per_msec()
{
    ...
    int ret;
    return NLOOP_FOR_ESTIMATION * NSECS_PER_MSEC / diff_nesc(before, after);
}
...
static void child_fn(int id, struct timespec *buf, int nrecord, unsigned long nloop_per_resol, struct timespec start)
{
    ...
    for (i = 0; i < nrecord; i++){
        printf("%d\t%ld\t%d\n", id, diff_nesc(start, buf[i]) / NSECS_PER_MSEC, (i + 1) * 100 / nrecord);
    }
    exit(EXIT_SUCCESS);
}
